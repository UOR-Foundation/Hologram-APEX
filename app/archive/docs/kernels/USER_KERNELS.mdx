---
title: "User-Defined Kernels Guide"
description: "User-Defined Kernels Guide documentation"
---
...
// Automatically tries inline kernel first, falls back to circuit compiler if needed
ops::math::vector_add(&mut exec, &a, &b, &mut c, n)?;

// For f32 types with n <= 3072:
// 1. Try inline::vector_add() (zero FFI, ~33ns)
// 2. Falls back to circuit compiler if inline fails
```
...
| Stdlib ops (large/non-f32) | Circuit compiler | 1-5µs         | Fallback          |
...
- **Large operations**: Use circuit compiler (handles overflow)
...
- Priority system (inline first, circuit compiler fallback)
...
3. **Use circuit compiler fallback** (always available)
   - ✅ Execute via circuit compiler (1-5µs)
