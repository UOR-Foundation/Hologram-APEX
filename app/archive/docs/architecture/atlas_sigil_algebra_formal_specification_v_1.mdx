---
title: "Atlas Sigil Algebra — Formal Specification (v1.0)"
description: "Atlas Sigil Algebra — Formal Specification (v1.0) documentation"
---
...
The **visual** form encodes `(h₂,d,ℓ)` as *(cardinal notch, center marker, inner 8‑dot ring)*.
...
Operator `∘ : Σ×Σ → Σ` on **typed terms** (see §5). Reading: *first s₁, then s₂*.
...
- Each generator (see §5.1) has a *typing judgement* `Γ ⊢ op : A → B ▷ β`, where `β∈𝔹` is the **budget profile**.
- **Sequential rule:**
...
- **Normalization:** any composite `R^a T_k M^m (s₁ ⊗ ··· ⊗ s_n)` can be rewritten by *pushing transforms inside* until only atomic sigils carry `(h₂,d,ℓ)` updates. This yields a canonical form.
- **Distribution laws:**
...
- **Progress/Preservation:** typed programs either *step* or are *values*; budgets are preserved modulo accounted work.
