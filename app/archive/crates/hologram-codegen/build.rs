...
fn find_kernel_files(base_dir: &Path) -> Vec<PathBuf> {
    ...
    let onnx_base = base_dir.parent().unwrap().join("onnx");
    let search_dirs = vec![
        // Stdlib directories
        base_dir.join("activation"),
        base_dir.join("vector"),
        base_dir.join("matrix"),
        base_dir.join("quantum"),
        // ONNX directories
        onnx_base.join("core"),
        onnx_base.join("activation"),
        onnx_base.join("normalization"),
        onnx_base.join("pooling"),
        onnx_base.join("reduction"),
        onnx_base.join("shape"),
    ];
...
fn main() {
    ...
    println!("cargo:rerun-if-changed=../../schemas/onnx/");
...
fn generate_inline_kernel_from_schema(schema: &JsonSchema) -> String {
    ...
    for (i, param) in schema.kernel.params.iter().enumerate() {
        let rust_type = match &param.param_type {
            ParamType::DevicePtr => {
                if param.name.to_lowercase().starts_with('c') || param.name.to_lowercase() == "out" || param.name == "Y"
                {
                    "*mut f32".to_string()
                } else {
                    "*const f32".to_string()
