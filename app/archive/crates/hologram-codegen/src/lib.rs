...
/// WASM stub for KernelLibrary (dynamic loading not supported)
...
impl KernelRegistry {
    ...
    /// WASM stub for load_from_directory (dynamic loading not supported)
    #[cfg(target_arch = "wasm32")]
    pub fn load_from_directory(
        _path: impl AsRef<Path>,
    ) -> Result<Vec<(String, KernelHandle)>, Box<dyn std::error::Error>> {
        Err("Dynamic kernel loading is not supported on WebAssembly".into())
    }
...
/// WASM stub for execute_kernel (dynamic loading not supported)
#[cfg(target_arch = "wasm32")]
pub unsafe fn execute_kernel(_handle: KernelHandle, _params: &[u8]) -> Result<(), String> {
    Err(
        "Dynamic kernel execution is not supported on WebAssembly. Use WebGPU or WASM SIMD operations instead."
            .to_string(),
    )
}
...
/// WASM stub for register_all_kernels_from_directory (dynamic loading not supported)
#[cfg(target_arch = "wasm32")]
pub fn register_all_kernels_from_directory(
    _kernel_dir: impl AsRef<Path>,
) -> Result<Vec<(String, KernelHandle)>, Box<dyn std::error::Error>> {
    Err("Dynamic kernel loading is not supported on WebAssembly".into())
}
