...
use hologram_compiler::lang::{VectorCircuits, VectorOperation};
...
fn bench_compilation(c: &mut Criterion) {
    ...
    for (name, circuit) in circuits {
        group.bench_with_input(BenchmarkId::from_parameter(name), &circuit, |b, &circuit| {
            b.iter(|| CircuitCompiler::compile(black_box(circuit)));
        });
    }
...
fn bench_raw_compilation(c: &mut Criterion) {
    ...
    for (circuit, name) in circuits {
        group.bench_with_input(BenchmarkId::from_parameter(name), &circuit, |b, &circuit| {
            b.iter(|| CircuitCompiler::compile_raw(black_box(circuit)));
        });
    }
