...
use hologram_compiler::lang::parse;
use hologram_compiler::{Canonicalizer, CircuitCompiler};

/// Benchmark simple expression parsing
fn bench_parse_simple(c: &mut Criterion) {
    c.bench_function("parse_simple", |b| {
        b.iter(|| parse(black_box("mark@c21")).unwrap());
    });
}
...
fn bench_parse_sequential(c: &mut Criterion) {
    ...
    c.bench_function("parse_sequential", |b| {
        b.iter(|| parse(black_box(expr)).unwrap());
    });
}
...
fn bench_pattern_matching(c: &mut Criterion) {
    ...
    group.bench_function("h_squared", |b| {
        let expr = "copy@c05->c06 . mark@c21 . copy@c05->c06 . mark@c21";
        b.iter(|| {
            let phrase = parse(black_box(expr)).unwrap();
...
/// Benchmark CircuitCompiler compilation
fn bench_compiler(c: &mut Criterion) {
    let mut group = c.benchmark_group("compiler");

    group.bench_function("simple", |b| {
        let expr = "mark@c21";
        b.iter(|| CircuitCompiler::compile(black_box(expr)).unwrap());
    });

    group.bench_function("h_squared", |b| {
        let expr = "copy@c05->c06 . mark@c21 . copy@c05->c06 . mark@c21";
        b.iter(|| hologram_compiler::CircuitCompiler::compile(black_box(expr)).unwrap());
    });
