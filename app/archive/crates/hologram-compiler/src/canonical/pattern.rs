...
use crate::lang::ast::{Sequential, Term};
        ...
        match (self, term) {
            ...
            (
                ...
                let class_index = match target {
                    crate::class::ClassTarget::Single(sigil) => sigil.class_index,
                    crate::class::ClassTarget::Range(_) => return false, // Ranges don't match patterns yet
                    // For CopyPair, match against source class
                    crate::class::ClassTarget::CopyPair { src, .. } => src.class_index,
                    // For SwapPair, match against first class
                    crate::class::ClassTarget::SwapPair { a, .. } => a.class_index,
                    // For TripleClass (merge/split), match against primary class
                    crate::class::ClassTarget::TripleClass { primary, .. } => primary.class_index,
                };
