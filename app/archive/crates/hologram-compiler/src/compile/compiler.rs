//! Circuit Compiler → Backend Generator Compiler
//!
//! This module translates circuit sigil expressions into backend generator call sequences.
...
//! let compiled = CircuitCompiler::compile(circuit).unwrap();
//!
//! assert_eq!(compiled.calls.len(), 1);
//! assert!(matches!(compiled.calls[0], GeneratorCall::MergeRange { .. }));
//! ```

use crate::canonical::canonicalization::Canonicalizer;
use crate::class::{ClassRange, ClassTarget, Generator, Transform};
use crate::class_system::{apply_transforms, components_to_class_index, decode_class_index};
use crate::lang::ast::{Parallel, Phrase, Sequential, Term};
...
mod tests {
    ...
    fn test_compile_raw_no_canonicalization() {
        ...
        let calls = CircuitCompiler::compile_raw(circuit).unwrap();
    ...
    fn test_compile_range_unsupported_operations() {
        ...
        for circuit in circuits {
            let result = CircuitCompiler::compile(circuit);
