...
#[cfg(feature = "model-server")]
lazy_static::lazy_static! {
    /// Registry of all active inference engines (from hologram-models)
    pub(crate) static ref INFERENCE_ENGINE_REGISTRY: ParkingLotMutex<HashMap<u64, Arc<ParkingLotMutex<hologram_models::InferenceEngine>>>> =
...
/// Public alias for generate_handle (for use by model_server module)
#[cfg(feature = "model-server")]
...
pub fn clear_all_registries() {
    ...
    #[cfg(feature = "model-server")]
    INFERENCE_ENGINE_REGISTRY.lock().clear();
