...
pub fn buffer_fill(executor_handle: u64, buffer_handle: u64, value: f32, len: u32) {
    ...
    // Fill buffer by creating a vector and copying from slice
    let fill_data = vec![value; buffer.len()];
    buffer
        .copy_from_slice(executor, &fill_data)
...
pub fn buffer_copy(executor_handle: u64, src_handle: u64, dst_handle: u64, len: u32) {
    ...
    // Copy buffer contents by reading from source and writing to destination
    ...
    unsafe {
        ...
        // Copy via host memory (read from source, write to destination)
        let data = src.to_vec(executor).expect("Failed to read source buffer");
        dst.copy_from_slice(executor, &data)
            .expect("Failed to copy to destination buffer");
    }
