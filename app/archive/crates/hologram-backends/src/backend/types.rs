...
/// Gauge metadata for period-aware optimizations
///
/// Gauge metadata from hologram-memory-manager's chunking system.
/// Backends can use this to enable period-aware optimizations during execution.
///
/// # Theory
///
/// A gauge defines:
...
/// - **Prime Factors**: Supported period decomposition
///
/// # Usage
///
/// When memory-manager detects periodicities in data, it constructs a gauge
/// that describes the period structure. This metadata flows to backends
/// for potential optimization (e.g., vectorization hints, prefetching patterns).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct GaugeMetadata {
    /// Cycle length: logical address space
    pub cycle_length: u64,

    /// Class count: number of resonance classes
    pub class_count: u16,

    /// Detected period (primorial: 30, 210, 2310, ...)
    pub period: u64,
}

impl GaugeMetadata {
    /// Create new gauge metadata
    pub const fn new(cycle_length: u64, class_count: u16, period: u64) -> Self {
        Self {
            cycle_length,
            class_count,
            period,
    ...
    pub const fn gauge_23(period: u64) -> Self {
        Self {
            cycle_length: 768,
            class_count: 96,
            period,
    ...
    pub const fn gauge_235(period: u64) -> Self {
        Self {
            cycle_length: 3840,
            class_count: 120,
            period,
...
/// Execution parameters for program execution
///
/// Combines launch configuration with optional initial register values and gauge metadata.
/// This allows precompiled programs to receive runtime parameters (e.g., buffer handles)
/// via register initialization before execution begins, and backends can leverage gauge
/// information for period-aware optimizations.
///
/// # Example
///
/// ```
/// use hologram_backends::{ExecutionParams, LaunchConfig, GaugeMetadata, Register};
///
/// let params = ExecutionParams::new(LaunchConfig::linear(1024, 256))
///     .with_register(Register(1), 12345)  // Buffer A handle
///     .with_register(Register(2), 67890)  // Buffer B handle
///     .with_gauge(GaugeMetadata::gauge_23(30)); // Period-aware execution
/// ```
#[derive(Debug, Clone)]
pub struct ExecutionParams {
    /// Launch configuration (grid, block, shared memory)
    pub launch_config: LaunchConfig,

    /// Initial register values to set before program execution
    ///
    /// These are applied to ALL lanes before the first instruction executes.
    /// Typically used to pass buffer handles (R1, R2, R3) and constants (R250+).
    pub initial_registers: std::collections::HashMap<crate::isa::Register, u64>,

    /// Optional gauge metadata for period-aware optimizations
    ///
    /// When set, backends can use this to optimize execution based on detected
    /// periodicities in the data (e.g., vectorization hints, prefetch patterns).
    pub gauge: Option<GaugeMetadata>,
}

impl ExecutionParams {
    /// Create new execution parameters with launch configuration
    pub fn new(launch_config: LaunchConfig) -> Self {
        Self {
            launch_config,
            initial_registers: std::collections::HashMap::new(),
            gauge: None,
        }
    }

    /// Add a register initialization (builder pattern)
    pub fn with_register(mut self, register: crate::isa::Register, value: u64) -> Self {
        self.initial_registers.insert(register, value);
        self
    }

    /// Set a register initialization (mutable)
    pub fn set_register(&mut self, register: crate::isa::Register, value: u64) {
        self.initial_registers.insert(register, value);
    }

    /// Set gauge metadata (builder pattern)
    pub fn with_gauge(mut self, gauge: GaugeMetadata) -> Self {
        self.gauge = Some(gauge);
        self
    }

    /// Set gauge metadata (mutable)
    pub fn set_gauge(&mut self, gauge: GaugeMetadata) {
        self.gauge = Some(gauge);
    }
}

impl From<LaunchConfig> for ExecutionParams {
    fn from(launch_config: LaunchConfig) -> Self {
        Self::new(launch_config)
    }
}
