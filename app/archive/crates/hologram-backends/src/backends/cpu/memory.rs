...
pub struct MemoryManager {
    ...
    /// Wrapped in RwLock for interior mutability with &self methods
    hot_pool: RwLock<Option<HotClassPool>>,

    /// Boundary pool (L2/L3 cache, 1.125 MB, 96 classes)
    /// Wrapped in RwLock for interior mutability with &self methods
    boundary_pool: RwLock<Option<BoundaryPool>>,
    ...
    /// This allows multiple blocks to read/write different buffers simultaneously
    ...
    next_buffer_id: Arc<parking_lot::Mutex<u64>>,

    /// Next pool handle ID
    next_pool_id: Arc<parking_lot::Mutex<u64>>,
}
...
impl MemoryManager {
    ...
    pub fn new() -> Self {
        Self {
            hot_pool: RwLock::new(None),
            boundary_pool: RwLock::new(None),
            class_access_counts: [0; CLASS_COUNT],
            total_accesses: 0,
            buffers: Arc::new(DashMap::new()),
            pools: Arc::new(DashMap::new()),
            next_buffer_id: Arc::new(parking_lot::Mutex::new(1)),
            next_pool_id: Arc::new(parking_lot::Mutex::new(1)),
        }
    }
    ...
    fn ensure_boundary_pools_initialized(&self) -> bool {
        // Already initialized?
        if self.boundary_pool.read().is_some() {
            return true;
        }

        // Acquire write lock for initialization
        let mut boundary_pool_guard = self.boundary_pool.write();

        // Double-check after acquiring lock (another thread may have initialized)
        if boundary_pool_guard.is_some() {
            return true;
        }
        ...
        *boundary_pool_guard = boundary_pool;
        *self.hot_pool.write() = hot_pool;
        true
    }

    /// Store to boundary class (boundary pool only, hot pool is read-only cache)
    pub fn store_boundary_class(&self, class: u8, offset: usize, src: &[u8]) -> Result<()> {
        ...
        let mut boundary_pool_guard = self.boundary_pool.write();
        if let Some(ref mut boundary_pool) = *boundary_pool_guard {
    ...
    /// Allocate a buffer (thread-safe via atomic ID generation and DashMap)
    pub fn allocate_buffer(&self, size: usize) -> Result<BufferHandle> {
        let mut id_lock = self.next_buffer_id.lock();
        let id = *id_lock;
        *id_lock += 1;
        drop(id_lock);

        // LOCK-FREE + PER-BUFFER: DashMap for concurrent access, RwLock for each buffer
        self.buffers.insert(id, Arc::new(RwLock::new(vec![0u8; size])));

        Ok(BufferHandle::new(id))
    }

    /// Free a buffer (thread-safe via DashMap)
    ...
    pub fn copy_to_buffer(&self, handle: BufferHandle, data: &[u8]) -> Result<()> {
        ...
        // LOCK-FREE + PER-BUFFER: Get buffer Arc from DashMap, then acquire write lock
        let buffer_arc = self
            .buffers
            .get(&handle.id())
    ...
    pub fn copy_from_buffer(&self, handle: BufferHandle, data: &mut [u8]) -> Result<()> {
        ...
        if handle.id() >= BOUNDARY_POOL_HANDLE_BASE {
            ...
            let boundary_pool_guard = self.boundary_pool.read();
            if boundary_pool_guard.is_none() {
                // Pools not yet initialized - return zeros (will be initialized on first write)
                drop(boundary_pool_guard);
                data.fill(0);
                return Ok(());
            }

            // Read specific class from boundary pool
            let boundary_pool = boundary_pool_guard.as_ref().unwrap();
            boundary_pool.load(class, 0, data)?;

            return Ok(());
        }

        // LOCK-FREE + PER-BUFFER: Get buffer Arc from DashMap, then acquire read lock
        let buffer_arc = self
            .buffers
            .get(&handle.id())
            .ok_or_else(|| BackendError::InvalidBufferHandle(handle.id()))?
    ...
    pub fn buffer_size(&self, handle: BufferHandle) -> Result<usize> {
        ...
        // LOCK-FREE + PER-BUFFER: Get buffer Arc from DashMap, then read size
        self.buffers
            .get(&handle.id())
            .map(|ref_multi| ref_multi.value().read().len())
            .ok_or_else(|| BackendError::InvalidBufferHandle(handle.id()))
    }

    /// Get Arc reference to buffer lock for direct access
    ///
    /// This allows callers to acquire their own read/write locks.
    /// Used by inline SIMD kernel fast paths that need direct pointer access.
    ///
    /// # Example
    ///
    /// ```ignore
    /// let buffer_lock = manager.get_buffer_lock(handle)?;
    /// let buffer = buffer_lock.read();
    /// let ptr = buffer.as_ptr();
    /// ```
    pub fn get_buffer_lock(&self, handle: BufferHandle) -> Result<Arc<RwLock<Vec<u8>>>> {
        self.buffers
            .get(&handle.id())
    ...
    /// Get raw const pointer to buffer memory (UNSAFE - lock must be held externally)
    ///
    /// # Safety
    ///
    /// DEPRECATED: This method is unsafe with per-buffer locking.
    /// Caller must hold a read lock on the buffer for the lifetime of the pointer.
    /// Use `get_buffer_lock()` instead.
    ///
    /// The returned pointer is valid as long as:
    /// - The buffer handle is valid
    /// - A read or write lock is held on the buffer
    /// - The MemoryManager is not dropped
    pub fn buffer_as_ptr(&self, handle: BufferHandle) -> Result<*const u8> {
        // Special handling for boundary pool handles
        const BOUNDARY_POOL_HANDLE_BASE: u64 = u64::MAX - 95;
        if handle.id() >= BOUNDARY_POOL_HANDLE_BASE {
            let class = (handle.id() - BOUNDARY_POOL_HANDLE_BASE) as u8;
            return self.boundary_class_ptr(class);
        }

        let buffer_lock = self.get_buffer_lock(handle)?;
        let buffer = buffer_lock.read();
        Ok(buffer.as_ptr())
    }

    /// Get raw mutable pointer to buffer memory (UNSAFE - lock must be held externally)
    ///
    /// # Safety
    ///
    /// DEPRECATED: This method is unsafe with per-buffer locking.
    /// Caller must hold a write lock on the buffer for the lifetime of the pointer.
    /// Use `get_buffer_lock()` instead.
    ///
    /// The returned pointer is valid as long as:
    /// - The buffer handle is valid
    /// - A write lock is held on the buffer
    /// - The MemoryManager is not dropped
    pub fn buffer_as_mut_ptr(&self, handle: BufferHandle) -> Result<*mut u8> {
        ...
        if handle.id() >= BOUNDARY_POOL_HANDLE_BASE {
            let class = (handle.id() - BOUNDARY_POOL_HANDLE_BASE) as u8;
            return self.boundary_class_ptr_mut(class);
        }
    ...
    pub fn boundary_class_ptr_mut(&self, class: u8) -> Result<*mut u8> {
        let mut boundary_pool_guard = self.boundary_pool.write();
        boundary_pool_guard
            .as_mut()
            .ok_or_else(|| BackendError::ExecutionError("Boundary pool not initialized".to_string()))?
    ...
    /// Allocate a pool (thread-safe via atomic ID generation and DashMap)
    pub fn allocate_pool(&self, size: usize) -> Result<PoolHandle> {
        let mut id_lock = self.next_pool_id.lock();
        let id = *id_lock;
        *id_lock += 1;
        drop(id_lock);

        // PER-POOL LOCKING: Each pool has its own RwLock
        self.pools.insert(id, Arc::new(RwLock::new(LinearPool::new(size))));
    ...
    pub fn copy_to_pool(&self, handle: PoolHandle, offset: usize, data: &[u8]) -> Result<()> {
        ...
        // PER-POOL LOCKING: Get pool Arc from DashMap, then acquire write lock
        let pool_arc = self
            .pools
            .get(&handle.id())
    ...
    pub fn copy_from_pool(&self, handle: PoolHandle, offset: usize, data: &mut [u8]) -> Result<()> {
        ...
        if handle.id() == 0 {
            ...
            if let Some(ref boundary_pool) = *boundary_pool_guard {
        ...
        // PER-POOL LOCKING: Get pool Arc from DashMap, then acquire read lock
        let pool_arc = self
            .pools
            .get(&handle.id())
            .ok_or_else(|| BackendError::InvalidPoolHandle(handle.id()))?
    ...
    pub fn pool_size(&self, handle: PoolHandle) -> Result<usize> {
        ...
        // PER-POOL LOCKING: Get pool Arc from DashMap, then read capacity
        self.pools
            .get(&handle.id())
            .map(|ref_multi| ref_multi.value().read().capacity())
            .ok_or_else(|| BackendError::InvalidPoolHandle(handle.id()))
    }
}
...
impl MemoryStorage for MemoryManager {
    ...
    /// Zero-copy buffer read via callback (CPU backend optimization)
    ///
    /// Acquires read lock and provides direct access to buffer memory.
    /// This is truly zero-copy - no data movement, just pointer casting.
    ...
    /// Zero-copy buffer write via callback (CPU backend optimization)
    ///
    /// Acquires write lock and provides direct mutable access to buffer memory.
    /// This is truly zero-copy - no data movement, just pointer casting.
