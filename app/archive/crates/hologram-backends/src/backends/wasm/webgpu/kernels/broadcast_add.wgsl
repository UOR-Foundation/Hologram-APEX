// Broadcast addition compute shader
...
fn compute_linear_index(coords: array<u32, 8>, shape: array<u32, 8>, ndim: u32) -> u32 {
    ...
    // Iterate from rightmost dimension (fastest changing)
...
fn clamp_broadcast_coords(coords: array<u32, 8>, shape: array<u32, 8>, ndim: u32) -> array<u32, 8> {
    ...
    for (var i: u32 = 0u; i < ndim; i = i + 1u) {
        // If dimension size is 1, clamp coordinate to 0 (broadcast)
        if (shape[i] == 1u) {
            result[i] = 0u;
        } else {
            result[i] = coords[i];
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Build shape arrays from params
    ...
    // Map to input A coordinates (with broadcasting)
    ...
    // Map to input B coordinates (with broadcasting)
