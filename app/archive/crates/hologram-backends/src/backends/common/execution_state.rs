...
impl<M: MemoryStorage> SharedExecutionState<M> {
    ...
    pub fn new(memory: Arc<M>, labels: HashMap<String, usize>) -> Self {
        Self {
            memory,
...
impl<M: MemoryStorage> ExecutionState<M> {
    ...
    pub fn new(num_lanes: usize, memory: Arc<M>, context: ExecutionContext, labels: HashMap<String, usize>) -> Self {
...
mod tests {
    ...
    impl MemoryStorage for MockStorage {
        fn allocate_buffer(&self, _size: usize) -> crate::error::Result<crate::backend::BufferHandle> {
            Ok(crate::backend::BufferHandle::new(0))
        }
        fn free_buffer(&self, _handle: crate::backend::BufferHandle) -> crate::error::Result<()> {
            Ok(())
        }
        fn copy_to_buffer(&self, _handle: crate::backend::BufferHandle, _data: &[u8]) -> crate::error::Result<()> {
            Ok(())
        }
        fn copy_from_buffer(
            &self,
        ...
        fn allocate_pool(&self, _size: usize) -> crate::error::Result<crate::backend::PoolHandle> {
            Ok(crate::backend::PoolHandle::new(0))
        }
        fn free_pool(&self, _handle: crate::backend::PoolHandle) -> crate::error::Result<()> {
            Ok(())
        }
        fn copy_to_pool(
            &self,
        ...
        fn with_buffer_read<F, R>(&self, _handle: crate::backend::BufferHandle, f: F) -> crate::error::Result<R>
        where
            F: FnOnce(&[u8]) -> crate::error::Result<R>,
        {
            f(&[])
        }
        fn with_buffer_write<F, R>(&self, _handle: crate::backend::BufferHandle, f: F) -> crate::error::Result<R>
        where
            F: FnOnce(&mut [u8]) -> crate::error::Result<R>,
        {
            f(&mut [])
        }
    }
    ...
    fn test_execution_state_creation() {
        let memory = Arc::new(MockStorage);
