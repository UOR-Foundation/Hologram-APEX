...
pub fn dispatch_common_instruction<M: super::MemoryStorage>(
    ...
    match instruction {
        ...
        // High-level operations (backend-specific, not in common instruction_ops)
        Instruction::Conv2d { .. }
        ...
        | Instruction::ParallelReduce { .. } => {
            use crate::error::BackendError;
            Err(BackendError::execution_error(
                "Conv2d, NearestUpsample2d, Gemm, ParallelMap, ParallelMapBinary, and ParallelReduce must be handled by backend executor".to_string(),
            ))
        }
