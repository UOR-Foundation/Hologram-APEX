...
/// # Thread Safety
///
/// Implementations must be thread-safe via interior mutability (e.g., DashMap, Arc<Mutex<_>>).
/// All methods take `&self` and use interior mutability for mutations.
/// The common execution state uses `Arc<M>` to share memory across lanes.
pub trait MemoryStorage: Send + Sync {
    ...
    /// Access buffer with zero-copy read (CPU backends only)
    ///
    /// Provides direct read access to buffer memory via callback.
    /// For CPU backends, this is zero-copy. For GPU backends, may require copy.
    ///
    /// # Arguments
    ///
    /// * `handle` - Buffer handle
    /// * `f` - Callback function that receives &[u8] slice
    ///
    /// # Returns
    ///
    ...
    /// Access buffer with zero-copy write (CPU backends only)
    ///
    /// Provides direct write access to buffer memory via callback.
    /// For CPU backends, this is zero-copy. For GPU backends, may require copy.
    ///
    /// # Arguments
    ///
    /// * `handle` - Buffer handle
    /// * `f` - Callback function that receives &mut [u8] slice
    ///
    /// # Returns
    ///
    /// Returns the result of the callback function
...
impl<S: MemoryStorage> MemoryStorage for MemoryManager<S> {
    ...
    where
        F: FnOnce(&[u8]) -> Result<R>,
    {
        self.storage.with_buffer_read(handle, f)
    }
    ...
    where
        F: FnOnce(&mut [u8]) -> Result<R>,
    {
        self.storage.with_buffer_write(handle, f)
    }
