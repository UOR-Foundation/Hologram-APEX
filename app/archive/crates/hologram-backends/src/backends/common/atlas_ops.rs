...
mod tests {
    ...
    impl MemoryStorage for MockStorage {
        fn allocate_buffer(&self, _size: usize) -> Result<BufferHandle> {
            Ok(BufferHandle::new(0))
        }
        fn free_buffer(&self, _handle: BufferHandle) -> Result<()> {
            Ok(())
        }
        fn copy_to_buffer(&self, _handle: BufferHandle, _data: &[u8]) -> Result<()> {
            Ok(())
        }
        ...
        fn allocate_pool(&self, _size: usize) -> Result<PoolHandle> {
            Ok(PoolHandle::new(0))
        }
        fn free_pool(&self, _handle: PoolHandle) -> Result<()> {
            Ok(())
        }
        fn copy_to_pool(&self, _handle: PoolHandle, _offset: usize, _data: &[u8]) -> Result<()> {
            Ok(())
        }
        ...
        fn with_buffer_read<F, R>(&self, _handle: BufferHandle, f: F) -> Result<R>
        where
            F: FnOnce(&[u8]) -> Result<R>,
        {
            f(&[])
        }
        fn with_buffer_write<F, R>(&self, _handle: BufferHandle, f: F) -> Result<R>
        where
