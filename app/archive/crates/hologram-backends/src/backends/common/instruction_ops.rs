...
pub fn execute_pow<M: MemoryStorage>(
    state: &mut ExecutionState<M>,
    ty: Type,
    dst: Register,
    src1: Register,
    ...
    match ty {
        Type::F32 => {
            let a = lane.registers.read_f32(src1)?;
            let b = lane.registers.read_f32(src2)?;
            lane.registers.write_f32(dst, a.powf(b))?;
        }
        Type::F64 => {
            let a = lane.registers.read_f64(src1)?;
            let b = lane.registers.read_f64(src2)?;
            lane.registers.write_f64(dst, a.powf(b))?;
        }
        _ => return Err(BackendError::UnsupportedOperation(format!("POW for type {}", ty))),
    }
    Ok(())
}
...
pub fn execute_pool_alloc<M: MemoryStorage>(state: &mut ExecutionState<M>, size: u64, dst: Register) -> Result<()> {
    // Allocate pool through memory manager (per-pool locking)
    let handle = state.shared.memory.as_ref().allocate_pool(size as usize)?;
...
pub fn execute_pool_free<M: MemoryStorage>(state: &mut ExecutionState<M>, handle: Register) -> Result<()> {
    ...
    // Free pool through memory manager (per-pool locking)
    state.shared.memory.as_ref().free_pool(PoolHandle::new(handle_id))?;
...
pub fn execute_pool_load<M: MemoryStorage>(
    ...
    // Load data from pool (per-pool locking)
    let value_bytes = {
        ...
        state
            ...
            .memory
            .as_ref()
            .copy_from_pool(PoolHandle::new(handle_id), offset_val as usize, &mut buffer)?;
        buffer
    };
...
pub fn execute_pool_store<M: MemoryStorage>(
    ...
    // Store data to pool (per-pool locking)
    state
        .shared
        .memory
        .as_ref()
        .copy_to_pool(PoolHandle::new(handle_id), offset_val as usize, &value_bytes)?;
...
///
/// For each position (batch, seq), normalizes across hidden_dim:
/// 1. Compute mean = sum(x) / hidden_dim
/// 2. Compute variance = sum((x - mean)^2) / hidden_dim
/// 3. Normalize: (x - mean) / sqrt(variance + eps)
/// 4. Apply scale and bias: gamma * normalized + beta
#[allow(clippy::too_many_arguments)]
pub fn execute_layer_norm<M: MemoryStorage>(
    state: &mut ExecutionState<M>,
    ty: Type,
    input_reg: Register,
    gamma_reg: Register,
    ...
    if ty != Type::F32 {
        return Err(BackendError::UnsupportedOperation(format!(
            "LAYER_NORM only supports f32, got {}",
            ty
        )));
    }
    ...
    // Read buffer handles from registers
    let input_handle = BufferHandle::new(lane.registers.read_u64(input_reg)?);
    let gamma_handle = BufferHandle::new(lane.registers.read_u64(gamma_reg)?);
    let beta_handle = BufferHandle::new(lane.registers.read_u64(beta_reg)?);
    let output_handle = BufferHandle::new(lane.registers.read_u64(output_reg)?);

    let inv_hidden_dim = 1.0 / (hidden_dim as f32);

    // Zero-copy access: nested callbacks for simultaneous buffer access
    // This keeps all data on-device for CPU backend (no host copies)
    state
        .shared
        .memory
        .as_ref()
        .with_buffer_read(input_handle, |input_bytes| {
            state
                .shared
                .memory
                .as_ref()
                .with_buffer_read(gamma_handle, |gamma_bytes| {
                    state
                        .shared
                        .memory
                        .as_ref()
                        .with_buffer_read(beta_handle, |beta_bytes| {
                            state
                                .shared
                                .memory
                                .as_ref()
                                .with_buffer_write(output_handle, |output_bytes| {
                                    // Cast byte slices to f32 slices (zero-copy pointer cast)
                                    let input_f32 = bytemuck::cast_slice::<u8, f32>(input_bytes);
                                    let gamma_f32 = bytemuck::cast_slice::<u8, f32>(gamma_bytes);
                                    let beta_f32 = bytemuck::cast_slice::<u8, f32>(beta_bytes);
                                    let output_f32 = bytemuck::cast_slice_mut::<u8, f32>(output_bytes);

                                    // Process each position independently
                                    for pos in 0..num_positions as usize {
                                        let offset = pos * hidden_dim as usize;
                                        let slice_start = offset;
                                        let slice_end = offset + hidden_dim as usize;

                                        // Step 1: Compute mean
                                        let sum: f32 = input_f32[slice_start..slice_end].iter().sum();
                                        let mean = sum * inv_hidden_dim;

                                        // Step 2: Compute variance
                                        let var_sum: f32 = input_f32[slice_start..slice_end]
                                            .iter()
                                            .map(|&x| {
                                                let diff = x - mean;
                                                diff * diff
                                            ...
                                            .sum();
                                        let variance = var_sum * inv_hidden_dim;

                                        // Step 3: Normalize and apply scale/bias
                                        let inv_std = (variance + eps).sqrt().recip();
                                        for i in 0..hidden_dim as usize {
                                            let input_idx = slice_start + i;
                                            let normalized = (input_f32[input_idx] - mean) * inv_std;
                                            output_f32[input_idx] = gamma_f32[i] * normalized + beta_f32[i];
                                        }
                                    }
...
/// For each batch and group, computes statistics over (channels_per_group, H, W).
#[allow(clippy::too_many_arguments)]
pub fn execute_group_norm<M: MemoryStorage>(
    state: &mut ExecutionState<M>,
    ty: Type,
    input_reg: Register,
    gamma_reg: Register,
    ...
    if ty != Type::F32 {
        return Err(BackendError::UnsupportedOperation(format!(
            "GROUP_NORM only supports f32, got {}",
            ty
        )));
    }
    ...
    let elements_per_group = (channels_per_group * height * width) as usize;
                        ...
                        .with_buffer_read(beta_handle, |beta_bytes| {
                            state
                                ...
                                .with_buffer_write(output_handle, |output_bytes| {
                                    ...
                                    // Helper: Calculate linear index for NCHW tensor
                                    let tensor_index = |n: usize, c: usize, h: usize, w: usize| -> usize {
                                        n * (num_channels as usize * spatial_size)
                                            + c * spatial_size
                                            + h * width as usize
                                            + w
                                    };
                                    ...
                                    for batch in 0..batch_size as usize {
                                        for group in 0..num_groups as usize {
                                            let channel_start = group * channels_per_group as usize;
                                            let channel_end = channel_start + channels_per_group as usize;

                                            // Step 1: Compute mean
                                            let mut sum = 0.0f32;
                                            for c in channel_start..channel_end {
                                                for h in 0..height as usize {
                                                    for w in 0..width as usize {
                                                        sum += input_f32[tensor_index(batch, c, h, w)];
                                                    }
                                                }
                                            }
                                            ...
                                            for c in channel_start..channel_end {
                                                for h in 0..height as usize {
                                                    for w in 0..width as usize {
                                                        let val = input_f32[tensor_index(batch, c, h, w)];
                                                        let diff = val - mean;
                                                        var_sum += diff * diff;
                                                    }
                                                }
                                            }
                                            let variance = var_sum / (elements_per_group as f32);

                                            // Step 3: Normalize and apply scale/bias per channel
                                            let inv_std = (variance + eps).sqrt().recip();
                                            for c in channel_start..channel_end {
                                                let gamma = gamma_f32[c];
                                                let beta = beta_f32[c];

                                                for h in 0..height as usize {
                                                    for w in 0..width as usize {
                                                        let idx = tensor_index(batch, c, h, w);
                                                        let normalized = (input_f32[idx] - mean) * inv_std;
                                                        output_f32[idx] = gamma * normalized + beta;
                                                    }
                                                }
                                            }
                                        }
...
/// Execute Batch Normalization instruction (inference mode)
///
/// Formula: output = scale * (input - mean) / sqrt(variance + eps) + bias
/// Normalizes per channel across batch and spatial dimensions in NCHW format
#[allow(clippy::too_many_arguments)]
pub fn execute_batch_norm<M: MemoryStorage>(
    state: &mut ExecutionState<M>,
    ty: Type,
    input_reg: Register,
    scale_reg: Register,
    ...
    if ty != Type::F32 {
        return Err(BackendError::UnsupportedOperation(format!(
            "BATCH_NORM only supports f32, got {}",
            ty
        )));
    }
    ...
    // Read buffer handles from registers
    let input_handle = BufferHandle::new(lane.registers.read_u64(input_reg)?);
    let scale_handle = BufferHandle::new(lane.registers.read_u64(scale_reg)?);
    let bias_handle = BufferHandle::new(lane.registers.read_u64(bias_reg)?);
    let mean_handle = BufferHandle::new(lane.registers.read_u64(mean_reg)?);
    let variance_handle = BufferHandle::new(lane.registers.read_u64(variance_reg)?);
    let output_handle = BufferHandle::new(lane.registers.read_u64(output_reg)?);
    ...
    state
        ...
        .with_buffer_read(input_handle, |input_bytes| {
            state
                ...
                .memory
                .as_ref()
                .with_buffer_read(scale_handle, |scale_bytes| {
                    state
                        .shared
                        .memory
                        .as_ref()
                        .with_buffer_read(bias_handle, |bias_bytes| {
                            state
                                .shared
                                .memory
                                .as_ref()
                                .with_buffer_read(mean_handle, |mean_bytes| {
                                    state
                                        .shared
                                        .memory
                                        .as_ref()
                                        .with_buffer_read(variance_handle, |variance_bytes| {
                                            state.shared.memory.as_ref().with_buffer_write(
                                                output_handle,
                                                |output_bytes| {
                                                    // Cast byte slices to f32 slices (zero-copy pointer cast)
                                                    let input_f32 = bytemuck::cast_slice::<u8, f32>(input_bytes);
                                                    let scale_f32 = bytemuck::cast_slice::<u8, f32>(scale_bytes);
                                                    let bias_f32 = bytemuck::cast_slice::<u8, f32>(bias_bytes);
                                                    let mean_f32 = bytemuck::cast_slice::<u8, f32>(mean_bytes);
                                                    let variance_f32 = bytemuck::cast_slice::<u8, f32>(variance_bytes);
                                                    let output_f32 = bytemuck::cast_slice_mut::<u8, f32>(output_bytes);

                                                    // Helper: Calculate linear index for NCHW tensor
                                                    let tensor_index =
                                                        |n: usize, c: usize, h: usize, w: usize| -> usize {
                                                            n * (num_channels as usize * spatial_size)
                                                                + c * spatial_size
                                                                + h * width as usize
                                                                + w
                                                    ...
                                                    for batch in 0..batch_size as usize {
                                                        for c in 0..num_channels as usize {
                                                            // Get pre-computed statistics for this channel
                                                            let mean_c = mean_f32[c];
                                                            let variance_c = variance_f32[c];
                                                            let scale_c = scale_f32[c];
                                                            let bias_c = bias_f32[c];

                                                            // Compute normalization factor
                                                            let inv_std = (variance_c + eps).sqrt().recip();

                                                            // Normalize all spatial locations for this channel
                                                            for h in 0..height as usize {
                                                                for w in 0..width as usize {
                                                                    let idx = tensor_index(batch, c, h, w);
                                                                    // BatchNorm formula: scale * (x - mean) / sqrt(var + eps) + bias
                                                                    let normalized =
                                                                        (input_f32[idx] - mean_c) * inv_std;
                                                                    output_f32[idx] = scale_c * normalized + bias_c;
                                                                }
                                                            }
                                                        }
                                                    }
