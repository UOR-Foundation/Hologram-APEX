...
impl CudaMemoryManager {
    ...
    /// Get device buffer pointer for kernel execution
    /// Returns the raw device pointer for passing to CUDA kernels
    pub fn get_device_buffer_ptr(&self, handle: u64) -> Result<&CudaSlice<u8>> {
        let (device_slice, _size) = self
            .buffers
            .get(&handle)
            .ok_or(BackendError::InvalidBufferHandle(handle))?;
        Ok(device_slice)
    }
