...
#[cfg(feature = "cuda")]
use cudarc::driver::{CudaDevice, CudaFunction, DevicePtr, LaunchAsync, LaunchConfig as CudaLaunchConfig};
...
pub struct CudaBackend {
    /// CUDA device (GPU)
    /// Used for kernel compilation and execution
    ...
    /// Maps kernel name → compiled function
    /// Loaded once at startup to avoid repeated PTX loading
    kernel_cache: RwLock<HashMap<String, Arc<CudaFunction>>>,

    /// PTX module loaded flag
    /// True if kernels have been loaded
    ptx_loaded: std::sync::atomic::AtomicBool,
}
...
impl CudaBackend {
    ...
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            ptx_loaded: std::sync::atomic::AtomicBool::new(false),
        })
    }
    ...
    /// Get device pointer for a buffer handle (for DLPack export)
    ///
    /// Returns the raw device pointer to the underlying CUDA memory.
    /// This is used for zero-copy interop with other frameworks via DLPack.
    pub fn get_device_ptr(&self, handle: u64) -> Result<*mut std::ffi::c_void> {
        let memory = self.memory.read();
        let cuda_slice = memory.get_device_buffer_ptr(handle)?;
        // cudarc::driver::CudaSlice has device_ptr() method that returns &CUdeviceptr
        let device_ptr = *cuda_slice.device_ptr() as *mut std::ffi::c_void;
        Ok(device_ptr)
    }

    /// Load PTX kernels and cache them
    ///
    /// This loads the compiled PTX module and caches all kernel functions.
    /// Called lazily on first kernel execution.
    fn ensure_kernels_loaded(&self) -> Result<()> {
        // Check if already loaded (fast path with atomic)
        if self.ptx_loaded.load(std::sync::atomic::Ordering::Acquire) {
            return Ok(());
        }

        // Load PTX module (slow path)
        ...
        // Load PTX from embedded bytes (compiled at build time)
        const PTX_SRC: &str = include_str!(concat!(env!("OUT_DIR"), "/atlas_kernels.ptx"));

        // Check if PTX is valid (not a placeholder)
        if PTX_SRC.contains("CUDA kernels not compiled") || PTX_SRC.contains("compilation failed") {
            return Err(BackendError::Other(
                "CUDA kernels not compiled. Install CUDA Toolkit and rebuild with nvcc in PATH.".into(),
            ));
        }

        // List of all kernel names we need to load
        ...
        // Load PTX module
        self.device
            .load_ptx(PTX_SRC.into(), "atlas_kernels", &kernel_names)
            .map_err(|e| BackendError::Other(format!("Failed to load PTX module: {}", e)))?;

        // Get and cache each kernel function
        for kernel_name in kernel_names {
            let func = self
                .device
                .get_func("atlas_kernels", kernel_name)
                .ok_or_else(|| BackendError::Other(format!("Failed to get kernel '{}'", kernel_name)))?;

            cache.insert(kernel_name.to_string(), Arc::new(func));
        }

        // Mark as loaded
        self.ptx_loaded.store(true, std::sync::atomic::Ordering::Release);

        Ok(())
    }

    /// Get a cached kernel function by name
    fn get_kernel(&self, name: &str) -> Result<Arc<CudaFunction>> {
        ...
        cache
            .get(name)
            .cloned()
            .ok_or_else(|| BackendError::Other(format!("Kernel '{}' not found in cache", name)))
    }
    ...
    fn execute_elementwise_binary(&mut self, pattern: &ElementWiseBinaryPattern) -> Result<()> {
        ...
        // Get kernel function from cache (loads PTX if needed)
        let func = self.get_kernel(&kernel_name)?;

        // Get device buffer pointers
        let memory = self.memory.read();
        let buf_a_ptr = memory.get_device_buffer_ptr(pattern.buffer_a)?;
        let buf_b_ptr = memory.get_device_buffer_ptr(pattern.buffer_b)?;
        let buf_c_ptr = memory.get_device_buffer_ptr(pattern.buffer_c)?;

        // Calculate grid/block dimensions
        // Use 256 threads per block (optimal for modern NVIDIA GPUs)
        let block_size = 256u32;
        let grid_size = ((pattern.n as u32) + block_size - 1) / block_size;
        ...
        // Kernel signature: (const T* a, const T* b, T* c, unsigned int n)
        unsafe {
            (*func)
                .clone()
                .launch(cfg, (buf_a_ptr, buf_b_ptr, buf_c_ptr, pattern.n as u32))
                .map_err(|e| BackendError::Other(format!("Failed to launch kernel '{}': {}", kernel_name, e)))?;
        }

        // Synchronize to ensure kernel completion
        self.device
            .synchronize()
            .map_err(|e| BackendError::Other(format!("Failed to synchronize device: {}", e)))?;
    ...
    fn execute_elementwise_unary(&mut self, pattern: &ElementWiseBinaryPattern) -> Result<()> {
        ...
        // Get device buffer pointers
        // For unary ops: buffer_a is input, buffer_c is output (buffer_b unused)
        let memory = self.memory.read();
        let buf_input_ptr = memory.get_device_buffer_ptr(pattern.buffer_a)?;
        let buf_output_ptr = memory.get_device_buffer_ptr(pattern.buffer_c)?;
        ...
        // Launch kernel with proper parameters
        // Kernel signature: (const T* input, T* output, unsigned int n)
        unsafe {
            (*func)
                .clone()
                .launch(cfg, (buf_input_ptr, buf_output_ptr, pattern.n as u32))
                .map_err(|e| BackendError::Other(format!("Failed to launch kernel '{}': {}", kernel_name, e)))?;
        }
