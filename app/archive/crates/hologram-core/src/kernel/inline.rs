...
///
/// Caller must ensure:
/// - `a` points to at least `n` valid f32 elements
/// - `b` points to at least `n` valid f32 elements
/// - `c` points to at least `n` valid, writable f32 elements
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
...
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn vector_mul(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
...
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn vector_sub(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
...
/// Inline vector_div implementation with SIMD acceleration
///
/// # Safety
///
/// Caller must ensure:
/// - `a` points to at least `n` valid f32 elements
/// - `b` points to at least `n` valid f32 elements (non-zero to avoid division by zero)
/// - `c` points to at least `n` valid, writable f32 elements
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn vector_div(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
            unsafe { vector_div_avx512(a, b, c, n) }
        } else if has_avx2 {
            unsafe { vector_div_avx2(a, b, c, n) }
        } else if has_sse4 {
            unsafe { vector_div_sse4(a, b, c, n) }
        } else {
            vector_div_scalar(a, b, c, n)
        }
    }
    #[cfg(not(any(target_arch = "x86_64", target_arch = "x86")))]
    {
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx512f")]
unsafe fn vector_div_avx512(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 16 * 16;
    for i in (0..simd_end).step_by(16) {
        let a_vec = _mm512_loadu_ps(a.add(i));
        let b_vec = _mm512_loadu_ps(b.add(i));
        let c_vec = _mm512_div_ps(a_vec, b_vec);
        _mm512_storeu_ps(c.add(i), c_vec);
    }
    for idx in simd_end..n {
        *c.add(idx) = *a.add(idx) / *b.add(idx);
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
unsafe fn vector_div_avx2(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 8 * 8;
    for i in (0..simd_end).step_by(8) {
        let a_vec = _mm256_loadu_ps(a.add(i));
        let b_vec = _mm256_loadu_ps(b.add(i));
        let c_vec = _mm256_div_ps(a_vec, b_vec);
        _mm256_storeu_ps(c.add(i), c_vec);
    }
    for idx in simd_end..n {
        *c.add(idx) = *a.add(idx) / *b.add(idx);
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse4.1")]
unsafe fn vector_div_sse4(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 4 * 4;
    for i in (0..simd_end).step_by(4) {
        let a_vec = _mm_loadu_ps(a.add(i));
        let b_vec = _mm_loadu_ps(b.add(i));
        let c_vec = _mm_div_ps(a_vec, b_vec);
        _mm_storeu_ps(c.add(i), c_vec);
    }
    for idx in simd_end..n {
        *c.add(idx) = *a.add(idx) / *b.add(idx);
...
fn vector_div_scalar(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    unsafe {
        for idx in 0..n {
            *c.add(idx) = *a.add(idx) / *b.add(idx);
        }
...
/// Inline abs implementation with SIMD acceleration
///
/// # Safety
///
/// Caller must ensure:
...
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn abs(a: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
            unsafe { abs_avx512(a, c, n) }
        } else if has_avx2 {
            unsafe { abs_avx2(a, c, n) }
        } else if has_sse4 {
            unsafe { abs_sse4(a, c, n) }
        } else {
            abs_scalar(a, c, n)
        }
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx512f")]
unsafe fn abs_avx512(a: *const f32, c: *mut f32, n: usize) {
    ...
    for i in (0..simd_end).step_by(16) {
        let a_vec = _mm512_loadu_ps(a.add(i));
        let c_vec = _mm512_and_ps(a_vec, sign_mask);
        _mm512_storeu_ps(c.add(i), c_vec);
    }

    // Handle remaining elements
    for idx in simd_end..n {
        *c.add(idx) = (*a.add(idx)).abs();
    }
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
unsafe fn abs_avx2(a: *const f32, c: *mut f32, n: usize) {
    // Create sign mask (all bits except sign bit)
    ...
    for i in (0..simd_end).step_by(8) {
        let a_vec = _mm256_loadu_ps(a.add(i));
        let c_vec = _mm256_and_ps(a_vec, sign_mask);
        _mm256_storeu_ps(c.add(i), c_vec);
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse4.1")]
unsafe fn abs_sse4(a: *const f32, c: *mut f32, n: usize) {
    // Create sign mask (all bits except sign bit)
    let sign_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7FFFFFFF));
    let simd_end = n / 4 * 4;

    // Process 4 elements at a time
    for i in (0..simd_end).step_by(4) {
        let a_vec = _mm_loadu_ps(a.add(i));
        let c_vec = _mm_and_ps(a_vec, sign_mask);
        _mm_storeu_ps(c.add(i), c_vec);
    }
...
fn abs_scalar(a: *const f32, c: *mut f32, n: usize) {
    unsafe {
        for idx in 0..n {
            *c.add(idx) = (*a.add(idx)).abs();
        }
...
/// Inline vector_max implementation with SIMD acceleration
///
/// # Safety
///
/// Caller must ensure:
...
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn vector_max(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
            unsafe { vector_max_avx512(a, b, c, n) }
        } else if has_avx2 {
            unsafe { vector_max_avx2(a, b, c, n) }
        } else if has_sse4 {
            unsafe { vector_max_sse4(a, b, c, n) }
        } else {
            vector_max_scalar(a, b, c, n)
        }
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx512f")]
unsafe fn vector_max_avx512(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 16 * 16;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm512_loadu_ps(a.add(i));
        let b_vec = _mm512_loadu_ps(b.add(i));
        let c_vec = _mm512_max_ps(a_vec, b_vec);
        _mm512_storeu_ps(c.add(i), c_vec);
        i += 16;
    }

    for idx in simd_end..n {
        *c.add(idx) = (*a.add(idx)).max(*b.add(idx));
    }
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
unsafe fn vector_max_avx2(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 8 * 8;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm256_loadu_ps(a.add(i));
        let b_vec = _mm256_loadu_ps(b.add(i));
        let c_vec = _mm256_max_ps(a_vec, b_vec);
        _mm256_storeu_ps(c.add(i), c_vec);
        i += 8;
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse4.1")]
unsafe fn vector_max_sse4(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 4 * 4;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm_loadu_ps(a.add(i));
        let b_vec = _mm_loadu_ps(b.add(i));
        let c_vec = _mm_max_ps(a_vec, b_vec);
        _mm_storeu_ps(c.add(i), c_vec);
        i += 4;
    }
...
#[inline(always)]
unsafe fn vector_max_scalar(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    for i in 0..n {
        *c.add(i) = (*a.add(i)).max(*b.add(i));
    }
}

/// Inline vector_min implementation with SIMD acceleration
///
/// # Safety
///
/// Caller must ensure:
...
/// - Memory regions do not overlap in invalid ways
#[inline(always)]
pub unsafe fn vector_min(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    #[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
    {
        let (has_avx512, has_avx2, has_sse4) = get_simd_caps();
        if has_avx512 {
            unsafe { vector_min_avx512(a, b, c, n) }
        } else if has_avx2 {
            unsafe { vector_min_avx2(a, b, c, n) }
        } else if has_sse4 {
            unsafe { vector_min_sse4(a, b, c, n) }
        } else {
            vector_min_scalar(a, b, c, n)
        }
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx512f")]
unsafe fn vector_min_avx512(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 16 * 16;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm512_loadu_ps(a.add(i));
        let b_vec = _mm512_loadu_ps(b.add(i));
        let c_vec = _mm512_min_ps(a_vec, b_vec);
        _mm512_storeu_ps(c.add(i), c_vec);
        i += 16;
    }

    for idx in simd_end..n {
        *c.add(idx) = (*a.add(idx)).min(*b.add(idx));
    }
}

#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "avx2")]
unsafe fn vector_min_avx2(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 8 * 8;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm256_loadu_ps(a.add(i));
        let b_vec = _mm256_loadu_ps(b.add(i));
        let c_vec = _mm256_min_ps(a_vec, b_vec);
        _mm256_storeu_ps(c.add(i), c_vec);
        i += 8;
    }
...
#[cfg(any(target_arch = "x86_64", target_arch = "x86"))]
#[target_feature(enable = "sse4.1")]
unsafe fn vector_min_sse4(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    let simd_end = n / 4 * 4;
    let mut i = 0;
    ...
    while i < simd_end {
        let a_vec = _mm_loadu_ps(a.add(i));
        let b_vec = _mm_loadu_ps(b.add(i));
        let c_vec = _mm_min_ps(a_vec, b_vec);
        _mm_storeu_ps(c.add(i), c_vec);
        i += 4;
    }
...
#[inline(always)]
unsafe fn vector_min_scalar(a: *const f32, b: *const f32, c: *mut f32, n: usize) {
    for i in 0..n {
        *c.add(i) = (*a.add(i)).min(*b.add(i));
    }
}
...
mod tests {
    ...
    fn test_vector_add_correctness() {
        ...
        for size in sizes {
            ...
            // Call SIMD function
            unsafe { vector_add(a.as_ptr(), b.as_ptr(), c.as_mut_ptr(), size) };
    ...
    fn test_vector_add_zeros() {
        ...
        for (i, &val) in c.iter().enumerate() {
            assert_eq!(val, 0.0, "vector_add zeros failed at index {}", i);
        }
    }
    ...
    fn test_vector_mul_correctness() {
        ...
        for size in sizes {
            ...
            unsafe { vector_mul(a.as_ptr(), b.as_ptr(), c.as_mut_ptr(), size) };
    ...
    fn test_vector_mul_by_zero() {
        ...
        for (i, &val) in c.iter().enumerate() {
            assert_eq!(val, 0.0, "vector_mul by zero failed at index {}", i);
        }
    }
    ...
    fn test_vector_sub_correctness() {
        ...
        for size in sizes {
            ...
            unsafe { vector_sub(a.as_ptr(), b.as_ptr(), c.as_mut_ptr(), size) };
    ...
    fn test_vector_sub_same_vectors() {
        ...
        unsafe { vector_sub(a.as_ptr(), a.as_ptr(), c.as_mut_ptr(), size) };
    ...
    fn test_vector_operations_large_values() {
        ...
        unsafe { vector_add(a.as_ptr(), b.as_ptr(), c_add.as_mut_ptr(), size) };
        unsafe { vector_mul(a.as_ptr(), b.as_ptr(), c_mul.as_mut_ptr(), size) };
        unsafe { vector_sub(a.as_ptr(), b.as_ptr(), c_sub.as_mut_ptr(), size) };
        // }
