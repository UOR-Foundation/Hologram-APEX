...
mod tests {
    ...
    fn test_chunk_size_constants() {
        ...
        const { assert!(PARALLEL_CHUNK_SIZE >= 1024) };
        const { assert!(PARALLEL_CHUNK_SIZE <= 16384) };

        // Threshold should be larger than chunk size
        const { assert!(PARALLEL_THRESHOLD > PARALLEL_CHUNK_SIZE) };
    }
    ...
    fn test_chunk_calculation() {
        ...
        for (n, expected_chunks) in test_cases {
            let chunks = n.div_ceil(PARALLEL_CHUNK_SIZE);
