...
//!
//! Provides PyTorch-compatible tensor semantics over Circuit buffers.
...
impl<T: bytemuck::Pod> Tensor<T> {
    ...
    /// ```
    pub async fn contiguous(&self, exec: &mut crate::executor::Executor) -> Result<Self>
    where
        T: Copy + std::fmt::Debug,
    {
        ...
        // Get full source buffer data (async on WASM)
        #[cfg(all(target_arch = "wasm32", feature = "webgpu"))]
        let full_src_data = self.buffer.to_vec_async(exec).await?;
    ...
    pub async fn reshape(&self, exec: &mut crate::executor::Executor, new_shape: Vec<usize>) -> Result<Self>
    where
        T: Copy + std::fmt::Debug,
    {
        ...
        if self.is_contiguous() {
            ...
            let contiguous = self.contiguous(exec).await?;
            Box::pin(contiguous.reshape(exec, new_shape)).await
        }
    }
    ...
    /// Concatenate this tensor with another along the specified dimension (synchronous version for native)
    ///
    /// For WASM/WebGPU, use the async version in hologram-onnx instead.
    /// This is kept simple for native (non-WASM) use.
    ///
    /// # Example
    ///
    /// ```text
    /// let a = Tensor::from_buffer(buf_a, vec![2, 3])?; // [2, 3]
    /// let b = Tensor::from_buffer(buf_b, vec![2, 3])?; // [2, 3]
    /// let c = a.concat_sync(&exec, &b, 0)?; // [4, 3]
    /// ```
    #[cfg(not(target_arch = "wasm32"))]
    pub fn concat_sync(&self, exec: &mut crate::executor::Executor, other: &Self, dim: usize) -> Result<Self> {
        if dim >= self.ndim() {
            return Err(Error::InvalidOperation(format!(
                "concat dimension {} out of range for {}D tensor",
                dim,
                self.ndim()
            )));
        }
        ...
        if self.ndim() != other.ndim() {
            return Err(Error::InvalidOperation(format!(
                "concat requires tensors with same number of dimensions, got {} and {}",
                self.ndim(),
                other.ndim()
            )));
        }

        // Check that all dimensions except concat dim match
        for i in 0..self.ndim() {
            if i != dim && self.shape[i] != other.shape[i] {
                return Err(Error::InvalidOperation(format!(
                    "concat dimension {} sizes must match, got {} and {}",
                    i, self.shape[i], other.shape[i]
                )));
            }
        }
        ...
        output_shape[dim] = self.shape[dim] + other.shape[dim];

        // Calculate total output size
        let output_size: usize = output_shape.iter().product();

        // Allocate output buffer
        let mut output_buffer = exec.allocate_class_free::<T>(output_size)?;

        // Copy data from both tensors (synchronous - native only)
        let self_data = self.buffer().to_vec(exec)?;
        let other_data = other.buffer().to_vec(exec)?;

        let mut output_data = Vec::with_capacity(output_size);

        // Simple concatenation along dimension dim
        let self_chunk_size: usize = self.shape[dim..].iter().product();
        let other_chunk_size: usize = other.shape[dim..].iter().product();
        let outer_iterations: usize = self.shape[..dim].iter().product();

        for outer_idx in 0..outer_iterations {
            // Copy chunk from self
            let self_start = outer_idx * self_chunk_size;
            let self_end = self_start + self_chunk_size;
            output_data.extend_from_slice(&self_data[self_start..self_end]);

            // Copy chunk from other
            let other_start = outer_idx * other_chunk_size;
            let other_end = other_start + other_chunk_size;
            output_data.extend_from_slice(&other_data[other_start..other_end]);
        }

        output_buffer.copy_from_slice(exec, &output_data)?;

        Self::from_buffer(output_buffer, output_shape)
    }

    /// Add a dimension of size 1 at the specified position (zero-copy)
    ///
    /// # Example
    ///
    /// ```text
    /// let tensor = Tensor::from_buffer(buffer, vec![77, 768])?; // [77, 768]
    /// let unsqueezed = tensor.unsqueeze(0)?; // [1, 77, 768]
    /// assert_eq!(unsqueezed.shape(), &[1, 77, 768]);
    /// ```
    pub fn unsqueeze(&self, dim: usize) -> Result<Self> {
        if dim > self.ndim() {
            return Err(Error::InvalidOperation(format!(
                "unsqueeze dimension {} out of range for {}D tensor (max {})",
                dim,
                self.ndim(),
                self.ndim()
            )));
        ...
        // New shape: insert dimension of size 1 at position `dim`
        let mut new_shape = Vec::with_capacity(self.ndim() + 1);
        let mut new_strides = Vec::with_capacity(self.ndim() + 1);

        // Copy dimensions before the new dimension
        for i in 0..dim {
            new_shape.push(self.shape[i]);
            new_strides.push(self.strides[i]);
        }
        ...
        new_shape.push(1);
        // Stride for size-1 dimension doesn't matter (no iteration), use 0 or copy adjacent stride
        let new_dim_stride = if dim < self.ndim() { self.strides[dim] } else { 1 };
        new_strides.push(new_dim_stride);
    ...
    /// let c = a.matmul(&mut exec, &b).await?;
    /// assert_eq!(c.shape(), &[2, 4]);
    /// ```
    pub async fn matmul(&self, _exec: &mut crate::executor::Executor, other: &Self) -> Result<Self> {
        // Validate 2D tensors
        ...
        // NOTE: ISA-based GEMM operation removed for MoonshineHRM refactor
        // Will be replaced with CompiledOperation in Phase 2.2
        Err(Error::NotImplemented(
            "matmul temporarily disabled during MoonshineHRM refactor (Phase 2.1). Will be restored with CompiledOperation in Phase 2.2".to_string()
        ))
    }
    ...
    ///
    /// Creates a DLManagedTensor that shares memory with this tensor. No data is copied.
    /// The tensor must be contiguous for DLPack export.
    ///
    /// # Safety
    ///
    /// The returned DLManagedTensor must not outlive the original tensor.
    /// The deleter will be called when the consuming framework is done.
    ///
    /// # Example
    ///
    /// ```ignore
    ...
    /// let dlpack = tensor.to_dlpack()?;
    /// // dlpack can be passed to PyTorch, JAX, etc. via PyCapsule
    /// ```
    pub fn to_dlpack(&self) -> Result<Box<crate::interop::DLManagedTensor>>
    where
        T: crate::interop::DLPackType,
    {
        ...
        // Note: This is a simplified version. Real implementation needs to:
        // 1. Get actual device pointer from executor/backend
        // 2. Handle GPU memory correctly
        // 3. Ensure proper lifetime management

        // For now, return error as we need executor access for proper implementation
        Err(Error::InvalidOperation(
            "DLPack export requires executor context. Use executor.to_dlpack(tensor) instead.".to_string(),
        ))
    }
}
...
mod tests {
    ...
    #[test]
    #[ignore = "Requires WebGPU backend for GPU-native GEMM"]
    ...
    fn test_tensor_matmul_dimension_mismatch() -> Result<()> {
        futures::executor::block_on(async {
            ...
            let result = a.matmul(&mut exec, &b).await;
            assert!(result.is_err());
            Ok(())
        })
    }
    ...
    fn test_tensor_contiguous_already_contiguous() -> Result<()> {
        ...
        let contiguous = futures::executor::block_on(tensor.contiguous(&mut exec))?;
    ...
    fn test_tensor_contiguous_after_transpose() -> Result<()> {
        ...
        // Make it contiguous
        let contiguous = futures::executor::block_on(transposed.contiguous(&mut exec))?;
        assert!(contiguous.is_contiguous());
