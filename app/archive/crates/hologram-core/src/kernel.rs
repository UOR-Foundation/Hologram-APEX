...
impl KernelLoader {
    ...
    /// # Platform Support
    ///
    /// This method is not supported on WebAssembly targets since dynamic library loading
    /// is not available in WASM. On WASM, this will return an error immediately.
    #[cfg(not(target_arch = "wasm32"))]
    ...
    /// Load kernels from a directory (WASM stub)
    ///
    /// # Platform Support
    ///
    /// Dynamic library loading is not supported on WebAssembly. This method always
    /// returns an error on WASM targets.
    #[cfg(target_arch = "wasm32")]
    pub fn load_from_directory(&self, _path: impl AsRef<Path>) -> Result<()> {
        Err(Error::InvalidOperation(
            "Dynamic kernel loading is not supported on WebAssembly. Use inline kernels or WebGPU backend instead."
                .to_string(),
        ))
    }
...
/// # Platform Support
///
/// This function is not available on WebAssembly targets. Use the standard
/// `ops::math::vector_add` operation instead, which will use WebGPU or WASM SIMD.
#[cfg(not(target_arch = "wasm32"))]
