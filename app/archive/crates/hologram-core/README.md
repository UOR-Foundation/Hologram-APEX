...
`hologram-core` provides general compute acceleration by compiling operations to their canonical forms using pattern-based canonicalization. This enables:

- **Canonical Compilation** - Operations compiled to optimal geometric representation
- **Pattern-Based Optimization** - Canonicalization reduces operations to minimal form
...
### 1. Canonical Compilation

The compiler uses **pattern-based rewriting** to reduce circuit operations to canonical form:
...
The system operates on **ClassMemory** with 96 classes:
...
All operations compile to canonical circuits.
...
Note: Some tests require full `CircuitExecutor` implementation.

## Dependencies

- `hologram-compiler` — Canonical circuit compilation
...
hologram-core is built for canonical circuit compilation:

**Core dependencies:**
- hologram-compiler - Canonical form compilation and circuit execution
- hologram-backends - Backend implementations
...
- **Hologram Compiler**: `crates/hologram-compiler/` — Canonical circuit compiler
- **Backend Architecture**: `docs/BACKEND_ARCHITECTURE.md` — Architecture overview
