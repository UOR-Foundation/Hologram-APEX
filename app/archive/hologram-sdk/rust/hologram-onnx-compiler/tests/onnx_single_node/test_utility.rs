...
use hologram_onnx_compiler::hrm::ops::{ArgMaxOp, ConstantOp, OnnxHRMNode, RangeOp, ShapeOp, SqueezeOp, TransposeOp};
...
mod test_constant {
    ...
    fn test_constant_no_inputs() {
        ...
        // Constant always returns same value regardless of inputs
        let dummy = vec![0.0];
        let result_with_input = constant_op.execute(&atlas, &[&dummy]).unwrap();
        assert_eq!(result_with_input, vec![1.0, 2.0, 3.0]);
    }
}
...
mod test_transpose {
    ...
    fn test_transpose_edge_cases() {
        ...
        let test_cases = vec![
            ...
            (vec![1, 4], None, vec![1.0, 2.0, 3.0, 4.0], vec![1.0, 2.0, 3.0, 4.0]),
            // 4x1 matrix (column vector)
            (vec![4, 1], None, vec![1.0, 2.0, 3.0, 4.0], vec![1.0, 2.0, 3.0, 4.0]),
            // Square matrix 2x2
            (vec![2, 2], None, vec![1.0, 2.0, 3.0, 4.0], vec![1.0, 3.0, 2.0, 4.0]),
        ];
