...
use hologram_onnx_compiler::proto::{AttributeProto, GraphProto, ModelProto, NodeProto, TypeProto, ValueInfoProto};
...
impl OnnxGraphBuilder {
    ...
    pub fn add_input(mut self, name: &str, shape: &[i64], dtype: DataType) -> Self {
        self.inputs.push(Self::create_value_info(name, shape, dtype));
        self
    }

    /// Add output tensor specification
    pub fn add_output(mut self, name: &str, shape: &[i64], dtype: DataType) -> Self {
        self.outputs.push(Self::create_value_info(name, shape, dtype));
        self
    }
    ...
    pub fn to_bytes(self) -> Result<Vec<u8>> {
        ...
        model
            .encode(&mut buf)
            .map_err(|e| hologram_onnx_compiler::CompilerError::ParseError(format!("Failed to encode: {}", e)))?;
        Ok(buf)
    }
    ...
    Op: OnnxHRMNode<T>,
{
    ...
    for test_case in test_cases {
        ...
        let result = op.execute(&atlas, &input_refs).map_err(|e| {
            hologram_onnx_compiler::CompilerError::ParseError(format!("Test case '{}' failed: {}", test_case.name, e))
        })?;
