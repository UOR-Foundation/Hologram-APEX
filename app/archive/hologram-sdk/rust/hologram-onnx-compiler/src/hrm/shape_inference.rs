...
impl ShapeInference {
    ...
    pub fn infer_shapes(
        ...
        for node_id in topo_order {
            ...
            let output_shapes = self.infer_op_output_shapes(&node_op_type, &input_shapes_for_op, &attributes)?;
    ...
    fn infer_op_output_shapes(
        ...
        match op_type {
            ...
            "Relu" | "Sigmoid" | "Tanh" | "Gelu" | "Neg" | "Abs" | "Sqrt" | "Exp" | "Log" => {
            ...
            "Softmax" => {
                if input_shapes.is_empty() {
                    return Err(CompilerError::InvalidModel("Softmax requires 1 input".to_string()));
                }
                // Output has same shape as input
                Ok(vec![input_shapes[0].clone()])
            }
            ...
            "Shape" => {
                ...
                // Output is a 1D tensor containing the input's shape
            ...
            "Cast" => {
                if input_shapes.is_empty() {
                    return Err(CompilerError::InvalidModel("Cast requires 1 input".to_string()));
                }
