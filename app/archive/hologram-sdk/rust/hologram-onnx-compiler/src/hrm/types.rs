...
pub enum DiscretizationStrategy {
    ...
    Quantized { bits: u8, scale: f32, zero_point: i32 },
...
impl ExtendedAddress {
    ...
    pub fn to_offset(&self, space: &AddressSpace) -> usize {
        let class_offset =
            self.class as usize * space.pages_per_class * space.bytes_per_page * space.sub_indices_per_byte;
...
pub struct PerfectHashTable {
    ...
    #[serde(serialize_with = "serialize_ahashmap", deserialize_with = "deserialize_ahashmap")]
...
fn serialize_ahashmap<S>(map: &AHashMap<u64, ExtendedAddress>, serializer: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    use serde::Serialize;
    let std_map: HashMap<u64, ExtendedAddress> = map.iter().map(|(&k, &v)| (k, v)).collect();
    ...
    D: serde::Deserializer<'de>,
{
    ...
    let std_map: HashMap<u64, ExtendedAddress> = HashMap::deserialize(deserializer)?;
...
impl PerfectHashTable {
    ...
    pub fn lookup(&self, hash: u64) -> crate::Result<ExtendedAddress> {
        self.entries
            .get(&hash)
            .copied()
            .ok_or_else(|| crate::CompilerError::MissingHashEntry(hash))
    }
}
...
impl OperationMetadataBuilder {
    ...
    pub fn with_input_shapes_from_stats(mut self, stats: &crate::hrm::types::OperationStats) -> Self {
    ...
    pub fn with_output_shapes_from_stats(mut self, stats: &crate::hrm::types::OperationStats) -> Self {
        self.output_shapes = stats.output_shapes.iter().map(|s| Some(s.clone())).collect();
        self
    }
    ...
    pub fn with_output_size_from_stats(mut self, stats: &crate::hrm::types::OperationStats) -> Self {
...
impl AddressSpace {
    ...
    pub fn with_mmap(capacity_bytes: usize, temp_dir: Option<&std::path::Path>) -> crate::Result<Self> {
        use std::fs::OpenOptions;

        // Create temporary file
        let dir = temp_dir.unwrap_or_else(|| std::path::Path::new("/tmp"));
        std::fs::create_dir_all(dir)
            .map_err(|e| crate::CompilerError::InvalidModel(format!("Failed to create temp dir: {}", e)))?;
        ...
        let file = OpenOptions::new()
            ...
            .truncate(true)
            .open(&file_path)
            .map_err(|e| crate::CompilerError::InvalidModel(format!("Failed to create mmap file: {}", e)))?;

        file.set_len(capacity_bytes as u64)
            .map_err(|e| crate::CompilerError::InvalidModel(format!("Failed to set mmap file size: {}", e)))?;

        // Create memory mapping
        let mmap = unsafe {
            memmap2::MmapMut::map_mut(&file)
                .map_err(|e| crate::CompilerError::InvalidModel(format!("Failed to create mmap: {}", e)))?
        };
    ...
    pub fn get_pointer(&self, address: ExtendedAddress) -> crate::Result<*const u8> {
        ...
        if offset >= self.data.len() {
            return Err(crate::CompilerError::InvalidAddress(format!("{:?}", address)));
        }
...
impl CompilationCheckpoint {
    ...
    pub fn save(&self, path: &std::path::Path) -> crate::Result<()> {
        let bytes = bincode::serialize(self).map_err(|e| {
            crate::CompilerError::BinaryGenerationError(format!("Checkpoint serialization failed: {}", e))
        })?;
    ...
    pub fn load(path: &std::path::Path) -> crate::Result<Self> {
        ...
        let checkpoint: Self = bincode::deserialize(&bytes)
            .map_err(|e| crate::CompilerError::ParseError(format!("Checkpoint deserialization failed: {}", e)))?;
