...
impl Compiler {
    ...
    pub fn compile(&self, input_path: impl AsRef<Path>, output_path: impl AsRef<Path>) -> Result<CompilationStats> {
        ...
        let (serializer_stats, ser_time) = self.serialize_binary(&graph, &exec_results, output_path.as_ref())?;
    ...
    fn load_onnx(&self, path: &Path) -> Result<(HologramGraph, HashMap<String, Vec<i64>>, std::time::Duration)> {
        let start = Instant::now();
        ...
        // Build HologramGraph (uses petgraph)
        let mut graph = HologramGraph::from_onnx(&onnx_graph)?;
    ...
    fn optimize_graph(&self, graph: &mut HologramGraph) -> Result<(OptimizationStats, std::time::Duration)> {
        let start = Instant::now();
    ...
    fn serialize_binary(
        ...
        if self.verbose {
            ...
            println!("   ✓ Total: {:.2} MB", stats.total_bytes as f64 / 1_000_000.0);
            println!("   ✓ Time: {:?}", elapsed);
        }
    ...
    /// Auto-detect input shapes for common models
    fn auto_detect_input_shapes_map(&self, graph: &HologramGraph) -> Result<HashMap<String, Vec<i64>>> {
