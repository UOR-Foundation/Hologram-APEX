...
fn benchmark_vector_add(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 3_072, 10_000, 1_000_000] {
        ...
        group.bench_with_input(BenchmarkId::new("inline_simd", size), &size, |b, &n| {
            ...
            b.iter(|| {
                let mut c = vec![0.0f32; n];
                unsafe { inline::vector_add(data_a.as_ptr(), data_b.as_ptr(), c.as_mut_ptr(), n) };
                black_box(c);
            });
        });
    }
...
fn benchmark_vector_mul(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 3_072, 10_000, 1_000_000] {
        ...
        group.bench_with_input(BenchmarkId::new("inline_simd", size), &size, |b, &n| {
            ...
            b.iter(|| {
                let mut c = vec![0.0f32; n];
                unsafe { inline::vector_mul(data_a.as_ptr(), data_b.as_ptr(), c.as_mut_ptr(), n) };
                black_box(c);
            });
        });
    }
...
fn benchmark_vector_sub(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 3_072, 10_000, 1_000_000] {
        ...
        group.bench_with_input(BenchmarkId::new("inline_simd", size), &size, |b, &n| {
            ...
            b.iter(|| {
                let mut c = vec![0.0f32; n];
                unsafe { inline::vector_sub(data_a.as_ptr(), data_b.as_ptr(), c.as_mut_ptr(), n) };
                black_box(c);
            });
        });
    }
    ...
    for size in [100, 1_000, 3_072, 10_000, 1_000_000] {
        ...
        group.bench_with_input(BenchmarkId::new("native_linear", size), &size, |b, &n| {
            ...
            b.iter(|| {
                ...
                for (i, &val) in data.iter().enumerate().take(n) {
                    if val == target {
                        results.push(i);
                    }
                }
