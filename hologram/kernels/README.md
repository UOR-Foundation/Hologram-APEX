...
Create `kernels/stdlib/vector/my_kernel.py`:
...
# 1. Run Python kernels → generate JSON
...
See `kernels/stdlib/vector/` for examples:
...
## ONNX Kernel Schemas (kernels/onnx/)

The `kernels/onnx/` directory contains ONNX operation schemas. These are provided as reference implementations but should be migrated to **hologram-onnx** (the external ONNX runtime crate).
...
2. **hologram-onnx** composes these primitives into ONNX operations

This keeps hologram-core focused on primitives while allowing hologram-onnx to provide optimized ONNX-specific implementations.

### ONNX Schema Organization
...
├── other/         # Cast, Shape, ArgMax
├── pooling/       # MaxPool, AvgPool
├── reduction/     # ReduceSum, ReduceMean
└── shape/         # Reshape, Transpose, Concat, Gather
```

### Migration Plan

These schemas should be moved to hologram-onnx where they can:
1. Use composition APIs (`DecomposeContext`, `Decomposable` trait)
2. Compose complex ops from hologram-core primitives
3. Provide ONNX-specific optimizations

See `docs/guides/composition.md` for examples of how to implement ONNX operations using composition.
