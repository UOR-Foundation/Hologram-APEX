...
def find_kernels_in_file(file_path: Path):
    ...
    # Read the module source for inline function support
    ...
    for name in dir(module):
        ...
        if callable(obj) and not name.startswith('_'):
            # Skip if it's marked as inline function
            if hasattr(obj, '_atlas_inline') and obj._atlas_inline:
                continue

            # Skip if function is imported from another module (not defined in this file)
            if hasattr(obj, '__module__') and obj.__module__ != 'kernel_module':
                continue
...
Examples:
  ...
  atlas-compile matrix_multiply.py -v
    ...
    # Find kernels
    kernels, module_source = find_kernels_in_file(Path(args.input))
    ...
    for name, func in kernels:
        try:
            ...
            schema = compiler.compile_function(func, module_source=module_source)
