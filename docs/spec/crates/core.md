...
- Data interchange: DLPack (ML frameworks) and Arrow (analytics/storage)
...
│   └── Memory (copy, fill)
└── Interop
    ├── DLPack (ML framework tensor interchange)
    └── Arrow (analytics & columnar storage)
```
...
#### HRM (Holographic Resonance Module)

The HRM module implements factorization via Monster group representation theory.

##### Declarative Requirements

All HRM operations MUST conform to Hologram's declarative programming model:

1. **O(1) complexity** - Table lookups only, no iteration over 196,884 dimensions
2. **Zero-allocation** - Precomputed const data, no runtime Vec/HashMap allocations
3. **Algebraically exact** - Cyclotomic field arithmetic, no lossy f64 approximation
4. **Build-time computed** - All values generated at compile time via build.rs

##### Algebraic Foundation

Operations use Monster group representation theory for O(1) complexity:

- **Character decomposition**: V = ⊕ᵢ Vᵢ (194 irreducible components)
- **Group action**: g·vᵢ = χᵢ(g)·vᵢ (O(194) scalar multiplications, not O(196,884))
- **Cyclotomic field**: Q(ζₙ) for exact arithmetic on Monster character values

##### Precomputed Data (const)

| Data | Format | Size | Generation |
|------|--------|------|------------|
| 96 rotation pairs | `[(f64, f64); 96]` | 1.5 KB | build.rs |
| 96 Lie generators | `[[f64; 196884]; 96]` | 151 MB | build.rs (compressed) |
| Structure constants | Sparse CSR | 120 KB | build.rs |
| Character table | `[[CyclotomicInteger; 194]; 194]` | 600 KB | GAP export |

##### Operations

```rust
pub mod hrm {
    /// Griess space dimension
    pub const GRIESS_DIMENSION: usize = 196_884;

    /// Number of resonance classes
    pub const NUM_RESONANCE_CLASSES: usize = 96;

    /// Griess vector Hadamard product via SIMD (O(1) dispatch)
    /// Uses precompiled ISA program, SIMD fast path
    pub fn griess_product(a: &GriessVector, b: &GriessVector) -> GriessVector;

    /// Griess dot product via SIMD reduction (O(1) dispatch)
    /// Uses precompiled ISA program with parallel reduction
    pub fn griess_dot(a: &GriessVector, b: &GriessVector) -> f64;

    /// Group action via character decomposition (O(194))
    /// NOT O(196,884) rotation - uses representation theory
    pub fn group_action(g: &MoonshineOperator, v: &GriessVector) -> GriessVector;

    /// Lie algebra scaling via precomputed generators (O(1) lookup)
    /// Uses ROTATION_TABLE[class] - no runtime sin/cos
    pub fn lie_scale(
        v: &GriessVector,
        generator_idx: usize,
        theta: f64,
    ...
    /// Precomputed rotation table (const)
    pub static ROTATION_TABLE: [(f64, f64); 96];

    /// Precomputed Lie generators (const, lazy-loaded from compressed)
    pub static LIE_GENERATORS: LazyLock<[[f64; GRIESS_DIMENSION]; 96]>;
}
```

##### Forbidden Patterns

These patterns are NOT ALLOWED in HRM code:

| Pattern | Why Forbidden | Replacement |
|---------|--------------|-------------|
| `for i in 0..GRIESS_DIMENSION` | O(n) iteration | ISA/SIMD fast path |
| `vec![0.0; GRIESS_DIMENSION]` | Runtime allocation | Precomputed const |
| `theta.cos()` / `theta.sin()` | Runtime transcendental | `ROTATION_TABLE[class]` |
| `.to_f64()` on CharacterValue | Lossy conversion | Cyclotomic arithmetic |
| `Vec::with_capacity(196_884)` | Dynamic allocation | Static buffer pool |

##### Cyclotomic Field Types

```rust
pub mod cyclotomic {
    /// Exact cyclotomic integer in Z[ζₙ]
    /// Represents: α = Σ cₖ · ζₙᵏ where cₖ ∈ Z and ζₙ = e^(2πi/n)
    ...
    impl CyclotomicInteger {
        /// Field addition (exact)
        pub fn add(&self, other: &Self) -> Self;

        /// Field multiplication (exact)
        pub fn mul(&self, other: &Self) -> Self;

        /// Complex conjugate: ζₙᵏ → ζₙ^(n-k)
        pub fn conjugate(&self) -> Self;

        /// Check if value is rational integer
        pub fn is_rational(&self) -> bool;

        /// Extract rational (returns None if irrational)
        pub fn as_rational(&self) -> Option<i128>;

        /// Lossy conversion (for final output ONLY)
        pub fn to_f64_lossy(&self) -> f64;
    }

    /// Sparse cyclotomic vector
    pub struct CyclotomicVector {
        coordinates: BTreeMap<usize, CyclotomicInteger>,
        dimension: usize,
    }

    /// Precomputed roots of unity
    pub struct RootsOfUnity {
        /// Exact cos(2πk/n) = (ζₙᵏ + ζₙ^(-k))/2
        pub fn cos_exact(&self, order: u32, k: u32) -> CyclotomicInteger;

        /// Exact sin(2πk/n) = (ζₙᵏ - ζₙ^(-k))/(2i)
        pub fn sin_exact(&self, order: u32, k: u32) -> CyclotomicInteger;
    }
}
```
...
The following must be generated at build time via `build.rs`:

```rust
// In build.rs
fn generate_hrm_precomputed_data() {
    let out_dir = env::var("OUT_DIR").expect("OUT_DIR not set");

    // 1. Rotation table: 96 sin/cos pairs
    generate_rotation_table(&out_dir);

    // 2. Lie generators: 96 × 196,884 f64 (compressed)
    generate_lie_generators(&out_dir);

    // 3. Structure constants: Sparse CSR format
    generate_structure_constants_csr(&out_dir);

    // 4. Canonical Atlas vectors
    generate_canonical_vectors(&out_dir);
}
```
...
##### DLPack Module (ML Framework Interchange)
...
##### Arrow Module (Analytics & Storage)
...
pub mod interop {
    pub mod arrow {
        ...
        /// Trait for types that can convert to/from Arrow arrays
        pub trait ArrowType: Copy + Send + Sync + 'static {
            type ArrowArray: Array;
            fn arrow_data_type() -> DataType;
            fn vec_to_arrow(data: Vec<Self>) -> Result<Self::ArrowArray>;
            fn arrow_to_vec(array: &Self::ArrowArray) -> Result<Vec<Self>>;
        }

        /// Extension trait for Buffer-Arrow conversion
        pub trait BufferArrowExt<T: ArrowType> {
            /// Export buffer to Arrow array (copy-based)
            fn to_arrow(&self, exec: &Executor) -> Result<T::ArrowArray>;

            /// Import from Arrow array (copy-based)
            fn from_arrow(exec: &mut Executor, array: &T::ArrowArray) -> Result<Buffer<T>>;
        }

        /// Extension trait for Tensor-Arrow conversion
        pub trait TensorArrowExt<T: ArrowType> {
            /// Export tensor to Arrow RecordBatch with shape metadata
            fn to_arrow_record_batch(&self, exec: &Executor) -> Result<RecordBatch>;

            /// Import from Arrow RecordBatch (reconstructs shape from metadata)
            fn from_arrow_record_batch(exec: &mut Executor, batch: &RecordBatch)
                -> Result<Tensor<T>>;

            /// Get flattened Arrow array (1D view)
            fn to_arrow_array(&self, exec: &Executor) -> Result<T::ArrowArray>;
        }

        /// BF16 support via UInt16 + metadata
        pub mod bf16_support {
            /// Convert bf16 Vec to Arrow UInt16Array with metadata
            pub fn bf16_to_arrow(data: Vec<bf16>)
                -> Result<(UInt16Array, HashMap<String, String>)>;

            /// Convert Arrow UInt16Array with bf16 metadata to bf16 Vec
            pub fn arrow_to_bf16(array: &UInt16Array, metadata: Option<&HashMap<String, String>>)
                -> Result<Vec<bf16>>;
        }
    }
}
...
| Hologram Type | Arrow DataType | Status |
|---------------|----------------|--------|
| f32, f64 | Float32/64 | ✅ Native |
| f16 | Float16 | ✅ Native (IEEE 754) |
| i8, i16, i32, i64 | Int8/16/32/64 | ✅ Native |
| u8, u16, u32, u64 | UInt8/16/32/64 | ✅ Native |
| **bf16** | UInt16 + metadata | ⚠️ Workaround |

**Use Cases:**
- **DLPack:** Runtime tensor interchange with ML frameworks (PyTorch, JAX) - zero-copy
- **Arrow:** Persistent storage (Parquet), analytics (Pandas, Polars), databases - columnar format
