...
- Three-phase operation framework (π → F → λ decomposition)
- Build-time kernel generation and dispatch
...
│   ├── Embedding strategies (π: input → torus)
│   ├── Operation types (F: torus operations)
│   ├── Lifting strategies (λ: torus → output)
│   ├── Operation catalog (24 standard operations)
│   ├── Kernel compiler (abstract kernel generation)
│   └── Dispatch table (288 precompiled kernels)
...
- [Atlas Specification](../atlas.md) - Three-phase decomposition framework and build-time kernel generation
